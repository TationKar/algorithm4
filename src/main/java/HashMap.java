...
public class HashMap<K, V> implements Iterable<HashMap.Entity> {
    ...
    class HashMapIterator implements Iterator<HashMap.Entity> {
        ...
        private Entity currrentEnt;

        public HashMapIterator(){
            currentIdx = 0;
            currrentEnt = buckets[currentIdx];
        }
        @Override
        public boolean hasNext() {
            while (currentIdx < size && currrentEnt == null) {
                currentIdx++;
                if (currentIdx < size) {
                    currrentEnt = buckets[currentIdx];
                }
            }
            return currentIdx < size && currrentEnt != null;
        }

        @Override
        public Entity next() {
            Entity value = (Entity) currrentEnt.value;
            currrentEnt = currrentEnt.next;
            if (currrentEnt == null && currentIdx < size) {
                currentIdx++;
                while (currentIdx < size && currrentEnt == null) {
                    currrentEnt = buckets[currentIdx];
                    currentIdx++;
